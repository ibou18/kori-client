...
export default function IdentityCheck() {
  ...
  const handleStatusChange = async () => {
    ...
    try {
      const payload = {
        status: statusAction === "verify" ? "verified" : "rejected",
        reason: statusAction === "reject" ? reason : null,
      };

      const response = await requestWrapper.patch(
        `/identity/verifications/${selectedVerification.id}/status`,
        payload
      );
          ...
          prevVerifications.map((v) =>
            v.id === selectedVerification.id
              ? {
                  ...
                  status: statusAction === "verify" ? "verified" : "rejected",
                  reason: statusAction === "reject" ? reason : null,
                  isIdentityVerified: statusAction === "verify",
                }
              : v
          )
        );
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={
                isSubmitting || (statusAction === "reject" && !reason.trim())
              }
              className={
                statusAction === "verify"
                  ...
                  <div className="space-y-4">
                    ...
                    {verification.status === "pending" && (
                      <div className="flex space-x-2 mt-4">
                        <Button
                          onClick={() =>
                            openStatusModal(verification, "verify")
                          }
                        ...
                        <Button
                          variant="destructive"
                          onClick={() =>
                            openStatusModal(verification, "reject")
                          }
                        >
